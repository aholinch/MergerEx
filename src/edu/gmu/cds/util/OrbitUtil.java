...
public class OrbitUtil
{
    ...
    public static double[][] computeOrbits(double rx, double ry, double vz, double m1, double m2, double d1)
    {
    	...
    	List<double[]> validRVs = new ArrayList<double[]>((int)(nr*nvm*nva/2));
    	double rvs[][] = null;
    	double tmpRv[] = null;
    ...
    */
    ...
    {
    	System.out.println(TU_s/(365.25*24.0d*3600.0d*1e6));
    	

        double r[] = {-3.665178117300539E+04,3.910221325001951E+05,-1.317619424083002E+04};
        double v[] = {-9.926911096311437E-01,-1.332004555316133E-01,-8.500217036840606E-02};
        double mu_earth = 4.0350323347908701E+05; //398600.4418;
        double coe[] = rvToCoe(r,v,mu_earth);
        double deg2ra = 180.0d/Math.PI;
        
        /*
 EC= 4.022896517726861E-02 QR= 3.712076545078842E+05 IN= 5.288736874971004E+00
 OM= 2.541053807247960E+02 W = 3.168743791456000E+02 Tp=  2455916.015739721712
 N = 1.513115974074710E-04 MA= 2.486709924165830E+02 TA= 2.444578129776006E+02
 A = 3.867668861005435E+05 AD= 4.023261176932029E+05 PR= 2.379196348251790E+06
         */
        
        System.out.println(coe[0] + "\t" + coe[0]/(1-coe[1]*coe[1]));
        System.out.println(coe[1]);
        System.out.println(coe[2]*deg2ra);
        System.out.println(coe[3]*deg2ra);
        System.out.println(coe[4]*deg2ra);
        System.out.println(coe[5]*deg2ra);
        System.out.println(coe[6]*deg2ra);
        double rv[] = coeToRv(coe,mu_earth);
        for(int i=0; i<rv.length; i++)System.out.println(rv[i]);
        
        double r2[] = {rv[0],rv[1],rv[2]};
        double v2[] = {rv[3],rv[4],rv[5]};
        
        System.out.println(MathUtil.mag(MathUtil.sub(r, r2)));
        System.out.println(MathUtil.mag(MathUtil.sub(v, v2)));
